...
public interface ExpenditureAuditLogDao {
    ...
    @Select("select * from expenditure_audit_log left join `user` on create_user = user where expenditure_id = #{expenditureId} and expenditure_audit_log.state = 1 order by ctime desc")
